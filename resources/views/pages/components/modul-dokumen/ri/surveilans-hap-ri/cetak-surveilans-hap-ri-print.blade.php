...
<x-pdf.layout-a4-with-out-background title="SURVEILANS HAIs — PNEUMONIA NON-VENTILATOR (HAP)">
    ...
    <table class="sv">
        <tr>
            ...
            <td class="lbl">Tirah Baring Mulai</td><td>{{ $nilai('tglMulaiTirahBaring') }}</td>
        </tr>
        <tr>
    ...
    <div class="sv-sec">3. TIRAH BARING &amp; TANDA KLINIS</div>
    <table class="sv">
        <tr>
            <td class="lbl">Tgl / Jam Mulai Tirah Baring</td><td>{{ $nilai('tglMulaiTirahBaring') }}</td>
            <td class="lbl">s/d Tgl / Jam Selesai</td><td>{{ $nilai('tglSelesaiTirahBaring') }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="lbl">Keterangan Sputum</td><td colspan="3">{{ $nilai('sputumKeterangan') }}</td>
        </tr>
    </table>

    {{-- 4. Kultur sputum --}}
    <div class="sv-sec">4. KULTUR SPUTUM</div>
    <table class="sv">
        @forelse ($form['kulturSputumHasil'] ?? [] as $i => $baris)
            <tr>
                <td class="lbl">Kultur ke-{{ $i + 1 }} — Tgl</td><td>{{ $baris['tgl'] ?: '-' }}</td>
                <td class="lbl">Hasil</td><td>{{ $baris['hasil'] ?: '-' }}</td>
            </tr>
        @empty
            <tr><td class="lbl">Hasil Kultur Sputum</td><td colspan="3">-</td></tr>
        @endforelse
    </table>
