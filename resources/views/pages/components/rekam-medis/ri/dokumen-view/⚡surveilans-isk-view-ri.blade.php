<?php
// Viewer read-only "Surveilans Infeksi Saluran Kemih" — display Rekam Medis RI.
...
new class extends Component {
    ...
    private string $printView = 'surveilans-isk-ri.cetak-surveilans-isk-ri-print';
    private string $filePrefix = 'surveilans-isk-ri';
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-surveilans-isk-ri-{$this->riHdrNo}");
    }
...
<div>
    <x-border-form title="Surveilans Infeksi Saluran Kemih">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" title="Surveilans Infeksi Saluran Kemih" :date="data_get($entri, 'tanggal')" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-surveilans-isk-ri-{{ $riHdrNo }}" title="Surveilans Infeksi Saluran Kemih"
