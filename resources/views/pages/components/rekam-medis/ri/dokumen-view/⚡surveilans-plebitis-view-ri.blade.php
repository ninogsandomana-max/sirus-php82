<?php
// Viewer read-only "Surveilans IADP & Plebitis" — display Rekam Medis RI.
...
new class extends Component {
    ...
    private string $printView = 'surveilans-plebitis-ri.cetak-surveilans-plebitis-ri-print';
    private string $filePrefix = 'surveilans-plebitis-ri';
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-surveilans-plebitis-ri-{$this->riHdrNo}");
    }
...
<div>
    <x-border-form title="Surveilans IADP & Plebitis">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" title="Surveilans IADP & Plebitis" :date="data_get($entri, 'tanggal')" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-surveilans-plebitis-ri-{{ $riHdrNo }}" title="Surveilans IADP & Plebitis"
