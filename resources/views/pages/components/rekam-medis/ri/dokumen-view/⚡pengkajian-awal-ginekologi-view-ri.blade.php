<?php
// Viewer read-only "Pengkajian Awal Ginekologi" (RM 45/45.a) — display Rekam Medis RI.
...
new class extends Component {
    ...
    private string $printView = 'pengkajian-awal-ginekologi-ri.cetak-pengkajian-awal-ginekologi-ri-print';
    private string $filePrefix = 'pengkajian-awal-ginekologi-ri';
    ...
    public function lihat(string $id): void
    {
        ...
        if (!$this->selected) {
            $this->dispatch('toast', type: 'error', message: 'Data pengkajian ginekologi tidak ditemukan.');
            return;
        }
        $this->previewHtml = $this->previewDokumenRi($this->selected, $this->printView, $this->ttdKey, $this->ttdCodeField);
        $this->dispatch('open-modal', name: "view-pengkajian-awal-ginekologi-ri-{$this->riHdrNo}");
    }
...
<div>
    <x-border-form title="Pengkajian Awal Ginekologi">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" title="Pengkajian Awal Ginekologi"
                :date="\Illuminate\Support\Str::before((string) data_get($entri, 'createdAt', ''), ' ')"
                :sub="\Illuminate\Support\Str::limit((string) (data_get($entri, 'diagnosa') ?: data_get($entri, 'keluhanUtama') ?: '-'), 70)" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-pengkajian-awal-ginekologi-ri-{{ $riHdrNo }}" title="Pengkajian Awal Ginekologi"
