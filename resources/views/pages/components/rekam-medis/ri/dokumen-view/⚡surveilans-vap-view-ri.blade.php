<?php
// Viewer read-only "Surveilans Pneumonia Ventilator (VAP)" — display Rekam Medis RI.
...
new class extends Component {
    ...
    private string $printView = 'surveilans-vap-ri.cetak-surveilans-vap-ri-print';
    private string $filePrefix = 'surveilans-vap-ri';
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-surveilans-vap-ri-{$this->riHdrNo}");
    }
...
<div>
    <x-border-form title="Surveilans Pneumonia Ventilator (VAP)">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" title="Surveilans Pneumonia Ventilator" :date="data_get($entri, 'tanggal')" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-surveilans-vap-ri-{{ $riHdrNo }}" title="Surveilans Pneumonia Ventilator"
