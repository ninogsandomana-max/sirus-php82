<?php
// Viewer read-only "Surveilans Infeksi Luka Operasi (ILO)" — display Rekam Medis RI.
...
new class extends Component {
    ...
    private string $printView = 'surveilans-ilo-ri.cetak-surveilans-ilo-ri-print';
    private string $filePrefix = 'surveilans-ilo-ri';
    ...
    public function lihat(string $id): void
    {
        ...
        $this->dispatch('open-modal', name: "view-surveilans-ilo-ri-{$this->riHdrNo}");
    }
...
<div>
    <x-border-form title="Surveilans Infeksi Luka Operasi (ILO)">
        @forelse (collect($list)->filter(fn($entri) => filled(data_get($entri, 'createdAt')))->values() as $entri)
            <x-rm.doc-list-row :id="data_get($entri, 'createdAt')" title="Surveilans Infeksi Luka Operasi" :date="data_get($entri, 'tanggalOperasi')" />
        @empty
            <x-rm.doc-empty />
        @endforelse
    </x-border-form>

    <x-rm.dokumen-view-modal name="view-surveilans-ilo-ri-{{ $riHdrNo }}" title="Surveilans Infeksi Luka Operasi"
        :subtitle="$selected ? (data_get($selected, 'tanggalOperasi') ?: null) : null"
