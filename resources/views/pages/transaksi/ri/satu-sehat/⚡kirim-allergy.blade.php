...
new class extends Component {
    ...
    /**
     * Isi yang AKAN dikirim, dari pengkajianDokter.anamnesa — sumber yang SAMA
     * dengan kirim(). "Tidak ada alergi" pun dikirim sebagai pernyataan negatif.
     */
    #[Computed]
    public function pratinjau(): array
    {
        ...
        $teks = trim((string) ($anamnesa['jenisAlergi'] ?? ''));
        $kode = trim((string) ($anamnesa['jenisAlergiSnomedCode'] ?? ''));
        if ($teks === '' && $kode === '') {
            return [];
        }
    ...
    public function kirim(string $riHdrNo): void
    {
        ...
        $this->dispatch('ri-satu-sehat.langkah-selesai', langkah: 'allergy');
    }
    ...
    public function kirimInti(string $riHdrNo): void
    {
        try {
            ...
            $alergiText = trim((string) ($anamnesa['jenisAlergi'] ?? ''));
            $snomedCode = trim((string) ($anamnesa['jenisAlergiSnomedCode'] ?? ''));
...
<div class="p-4 bg-canvas border border-hairline shadow-sm rounded-xl dark:bg-gray-900 dark:border-gray-700">
    ...
    <x-satu-sehat.pratinjau :terbuka="$pratinjauTerbuka"
        ...
        kosong="Data alergi belum diisi di Pengkajian Dokter — Kirim akan ditolak." />
</div>
