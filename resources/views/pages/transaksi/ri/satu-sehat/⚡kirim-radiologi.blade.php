...
new class extends Component {
    use EmrRITrait, ServiceRequestTrait, DiagnosticReportTrait, ObservationTrait, ImagingStudyTrait, OrthancTrait, PenunjangKirimTrait;
    ...
    public function pratinjau(): array
    {
        ...
        $order = DB::table('rstxn_riradiologs as a')
            ->leftJoin('rsmst_radiologis as m', 'a.rad_id', '=', 'm.rad_id')
            ->where('a.rihdr_no', $this->riHdrNo)
            ->orderBy('a.rirad_no')
            ->get(['a.rirad_no as urutan', 'a.rad_id', 'm.rad_desc']);
    ...
    public function kirim(string $riHdrNo): void
    {
        ...
        $this->dispatch('ri-satu-sehat.langkah-selesai', langkah: 'radiologi');
    }
    ...
    public function kirimInti(string $riHdrNo): void
    {
        try {
            ...
            $waktu        = $this->parseDate($dataRI['entryDate'] ?? '')->toIso8601String();

            $orders = DB::table('rstxn_riradiologs as a')
                ->leftJoin('rsmst_radiologis as m', 'a.rad_id', '=', 'm.rad_id')
                ->where('a.rihdr_no', $riHdrNo)
                ->select('a.rirad_no', 'a.rad_id', 'm.rad_desc', 'm.loinc_code', 'm.loinc_display', 'a.radnum_no', 'a.rad_upload_pdf_foto', 'a.study_uid')
                ->get();

            $regNo   = $dataRI['regNo'] ?? '';
            $regName = $dataRI['regName'] ?? '';
            ...
            foreach ($orders as $order) {
                ...
                if ($adaFoto && empty($this->idKirim($indeks, $kunciOrder, 'is'))) {
                    ...
                    $studyUid = $this->prosesOrthanc(
                        'rstxn_riradiologs',
                        ['rihdr_no' => $riHdrNo, 'rirad_no' => $nomorDetail],
                        [
                            'radnum_no'          => $radnumNo,
