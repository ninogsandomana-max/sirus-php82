    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 items-start">
        ...
        @if (!$isFormLocked)
            <div class="p-4 border border-hairline rounded-2xl dark:border-gray-700 bg-surface-soft dark:bg-gray-800/40"
                ...
                x-on:focus-lov-visit-ri.window="$nextTick(() => {
                const fokus = () => {
                    const el = $refs.lovVisit?.querySelector('input');
                ...
                <div x-ref="lovVisit"
                    x-on:keydown.enter="if (!$event.target.value?.trim()) $dispatch('administrasi-ri-goto-tab', { tab: 'RiKonsul', focus: 'focus-lov-konsul-ri' })">
                ...
                @else
                    <div class="grid grid-cols-4 gap-3 items-end">
                        ...
                        {{-- Tarif --}}
                        {{-- x-init TIDAK boleh dioper ke x-text-input-number: komponen sudah merender
                             x-init sendiri, atribut jadi ganda & yang menang milik komponen (autofokus mati). --}}
                        <div x-data x-init="$nextTick(() => { $refs.inputVisitPrice?.focus(); $refs.inputVisitPrice?.select(); })">
                            <x-input-label value="Tarif Kunjungan" class="mb-1" />
                            <x-text-input-number wire:model="formEntry.visitPrice" x-ref="inputVisitPrice"
                    ...
                    <tbody class="divide-y divide-hairline-soft dark:divide-gray-800">
                        @forelse ($dataDaftarRI['RiVisit'] ?? [] as $item)
                            <tr wire:key="visit-ri-{{ $item['visit_no'] ?? $loop->index }}" class="transition group hover:bg-surface-soft dark:hover:bg-gray-800/40">
                                <td class="px-4 py-1.5 font-mono text-sm text-muted whitespace-nowrap">{{ $item['visit_date'] ?? '-' }}</td>
                                <td class="px-4 py-1.5 text-ink dark:text-gray-200 whitespace-nowrap">{{ $item['dr_name'] ?? '-' }}</td>
                                ...
                                @if (!$isFormLocked)
                                    <td class="px-4 py-1.5 text-center">
                                        <x-outline-button type="button" wire:click.prevent="removeVisit({{ $item['visit_no'] }})"
                                            wire:confirm="Hapus kunjungan ini?"
                                            wire:loading.attr="disabled" wire:target="removeVisit({{ $item['visit_no'] }})"
