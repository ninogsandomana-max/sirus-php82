    ...
    public function kirimLaboratorium(): void
    {
        ...
        try {
            DB::transaction(function () use ($riData) {
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Order Lab' . ($this->cito === '1' ? ' [CITO]' : '') . ' — ' . collect($this->selectedItems)->pluck('clabitem_desc')->implode(', '), 'MR');
            });

            $this->dispatch('laborat-order-terkirim');
            $this->dispatch('toast', type: 'success', message: count($this->selectedItems) . ' item lab berhasil dikirim' . ($this->cito === '1' ? ' dengan status CITO.' : '.'));
            $this->closeModal();
