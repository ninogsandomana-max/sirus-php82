...
new class extends Component {
    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogRI((int) $this->riHdrNo, 'Buka kunci Catatan Terapi & Perencanaan Neonatal — ' . $jenisEntri . ' (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut', 'MR');
            });
    ...
    public function cetakSemua()
    {
        ...
        try {
            ...
            set_time_limit(300);
            $pdf = Pdf::loadView('pages.components.modul-dokumen.ri.catatan-terapi-neonatal-ri.cetak-catatan-terapi-neonatal-ri-print', ['data' => $data])->setPaper('A4');
                                                            ...
                                                            @if ($isFinal)
                                                                @can('dokumen.bukaKunci')
                                                                    <x-confirm-button action="bukaKunci('{{ $rowKey }}')"
                                                                        title="Buka Kunci Catatan Terapi & Perencanaan Neonatal"
