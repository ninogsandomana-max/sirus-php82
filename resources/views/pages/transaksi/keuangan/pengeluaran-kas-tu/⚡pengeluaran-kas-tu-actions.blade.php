...
new class extends Component {
    ...
    public function onAkunCoSelected(string $target, ?array $payload): void
    {
        ...
        $this->dispatch('focus-input-ket-co');
    }
    ...
    public function onAkunKasSelected(string $target, ?array $payload): void
    {
        ...
        $this->dispatch('focus-btn-simpan-co');
    }
    ...
    {
        return [
            'formEntry.accId.required' => 'Akun pengeluaran wajib dipilih.',
    ...
    public function openCreate(): void
    {
        ...
        $this->dispatch('focus-input-tanggal-co');
    }
    ...
    public function simpan(): void
    {
        ...
        try {
            ...
            $this->dispatch('toast', type: 'success', message: 'Pengeluaran kas berhasil disimpan.');
    ...
    {
        $row = DB::table('rstxn_tucashks')->where('tucashk_no', $this->editingNo)->first();
        ...
        try {
            ...
            $this->closeModal();
            $this->dispatch('pengeluaran-kas.saved');
            $this->dispatch('toast', type: 'success', message: 'Pengeluaran kas berhasil diperbarui.');
        } catch (QueryException $e) {
...
<div>
    <x-modal name="pengeluaran-kas-tu-actions" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            ...
            x-data
            x-on:focus-input-tanggal-co.window="$nextTick(() => setTimeout(() => $refs.inputTanggal?.focus(), 150))"
            x-on:focus-input-ket-co.window="$nextTick(() => setTimeout(() => { $refs.inputKet?.focus(); $refs.inputKet?.select(); }, 150))"
            x-on:focus-btn-simpan-co.window="$nextTick(() => setTimeout(() => $refs.btnSimpan?.focus(), 150))">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-2xl font-semibold text-ink dark:text-gray-100">
                                    {{ $editingNo ? "Koreksi Pengeluaran Kas #{$editingNo}" : 'Entri Pengeluaran Kas' }}
                                </h2>
            ...
            <div class="flex-1 px-4 py-4 bg-surface-soft/70 dark:bg-gray-950/20">
                <x-border-form title="Data Pengeluaran Kas (Cash-Out)" class="max-w-3xl">
                    <div class="space-y-4">
                        {{-- 1 & 2. Tanggal + Akun Pengeluaran — sejajar --}}
                            ...
                            <div class="sm:col-span-5">
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <x-text-input type="text" wire:model="formEntry.tucashkDate"
                                        ...
                                        x-on:keydown.enter.prevent="($refs.lovCoWrapper?.querySelector('input:not([disabled])') || $refs.inputKet)?.focus()" />
                                    <x-now-button wire:click.prevent="setTanggalSekarang" />
                                </div>
                                <x-input-error :messages="$errors->get('formEntry.tucashkDate')" class="mt-1" />
                            </div>

                            <div class="sm:col-span-7" x-ref="lovCoWrapper">
                                <livewire:lov.akun-co.lov-akun-co target="akun-co-tu" label="Akun Pengeluaran (CO)"
                                    :initialAccId="$formEntry['accId'] ?: null" :error="$errors->has('formEntry.accId')"
                                    wire:key="lov-co-entry-{{ $editingNo ?? 'new' }}-{{ $renderVersions['modal'] ?? 0 }}" />
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-12 items-start">
                            <div class="sm:col-span-8">
                                ...
                                <x-text-input type="text" wire:model="formEntry.tucashkDesc"
                                    placeholder="Keterangan pengeluaran kas" class="w-full mt-1 text-sm"
                        ...
                        <div x-ref="lovKasWrapper">
                            <livewire:lov.kas.lov-kas target="akun-kas-tu-co" tipe="" label="Akun Kas"
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 mt-5">
                        <p class="text-xs text-muted dark:text-gray-400">
                            ...
                            Tanggal → Akun Pengeluaran → Keterangan → Nominal → Akun Kas → tombol Simpan.
