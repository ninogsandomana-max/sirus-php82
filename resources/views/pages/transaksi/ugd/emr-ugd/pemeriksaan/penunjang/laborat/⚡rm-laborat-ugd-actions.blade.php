    ...
    public function kirimLaboratorium(): void
    {
        ...
        try {
            DB::transaction(function () use ($ugdData, $now) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Order Lab UGD' . ($this->cito === '1' ? ' [CITO]' : '') . ' — ' . $namaItems, 'MR');
            });

            $this->dispatch('toast', type: 'success', message: "{$itemCount} item laboratorium berhasil dikirim" . ($this->cito === '1' ? ' dengan status CITO.' : '.'));
