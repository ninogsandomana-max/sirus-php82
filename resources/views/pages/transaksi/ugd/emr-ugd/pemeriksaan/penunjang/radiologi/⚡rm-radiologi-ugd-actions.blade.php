...
new class extends Component {
    ...
    public function kirimRadiologi(): void
    {
        ...
        if (!$sudahDiperingatkan) {
            $ganda = OrderRadiologiGanda::cari('UGD', $this->rjNo, array_keys($this->selectedItems));
            if (!empty($ganda)) {
        ...
        try {
            DB::transaction(function () use ($now, $drPengirimName, $sudahDiperingatkan) {
                ...
                if (!$sudahDiperingatkan) {
                    $gandaKunci = OrderRadiologiGanda::cari('UGD', $this->rjNo, array_keys($this->selectedItems));
                    if (!empty($gandaKunci)) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Order Radiologi UGD' . ($this->cito === '1' ? ' [CITO]' : '') . ($sudahDiperingatkan ? ' [ULANG]' : '') . ' — ' . $namaItems, 'MR');
            });

            $this->dispatch('toast', type: 'success', message: "{$itemCount} item radiologi berhasil dikirim" . ($this->cito === '1' ? ' dengan status CITO.' : '.'));
