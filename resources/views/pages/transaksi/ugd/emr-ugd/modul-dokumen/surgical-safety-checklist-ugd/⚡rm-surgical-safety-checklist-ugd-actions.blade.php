<?php
// resources/views/pages/transaksi/ugd/emr-ugd/modul-dokumen/surgical-safety-checklist/rm-surgical-safety-checklist-ugd-actions.blade.php
// Surgical Safety Checklist — UGD (WHO SSC).
// Pola: multi-entri append-only (Draft + Lanjut Isi + TTD-Kunci + Lihat read-only + tabel expandable),
// disimpan ke datadaftarri_json (key surgicalSafetyChecklistUGD). Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-surgical-safety-checklist-ugd'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'surgicalSafetyChecklistUGD';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-surgical-safety-checklist-ugd']);
    ...
    public function openModal(): void
    {
        ...
        $this->surgicalSafetyChecklistList = $this->dataDaftarUGD[$this->jsonKey];
        ...
        $this->dispatch('open-modal', name: "rm-surgical-safety-checklist-ugd-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-surgical-safety-checklist-ugd-{$this->rjNo}");
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            set_time_limit(300);
            $pdf = Pdf::loadView('pages.components.modul-dokumen.ugd.surgical-safety-checklist-ugd.cetak-surgical-safety-checklist-ugd-print', ['data' => $data])->setPaper('A4');
            $this->dispatch('toast', type: 'success', message: 'Berhasil mencetak Surgical Safety Checklist.');
            return response()->streamDownload(fn() => print $pdf->output(), 'surgical-safety-checklist-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
...
<div>
    ...
    <x-modal name="rm-surgical-safety-checklist-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]" wire:key="{{ $this->renderKey('modal-surgical-safety-checklist-ugd', [$rjNo ?? 'new']) }}">
                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-base text-muted dark:text-gray-400">UGD — WHO SSC</p>
                            </div>
                        </div>
