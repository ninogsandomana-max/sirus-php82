...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-laporan-operasi-ugd'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'laporanOperasiUGD';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-laporan-operasi-ugd']);
    ...
    public function openModal(): void
    {
        ...
        $this->laporanList = $this->dataDaftarUGD[$this->jsonKey];
        $this->isFormLocked = $this->checkEmrUGDStatus($this->rjNo) || $this->disabled;
        $this->incrementVersion('modal-laporan-operasi-ugd');

        $this->dispatch('open-modal', name: "rm-laporan-operasi-ugd-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-laporan-operasi-ugd-{$this->rjNo}");
    }
    ...
    private function persistEntry(string $key, bool $finalized, string $logVerb): void
    {
        ...
        DB::transaction(function () use ($entry, $key, $logVerb) {
            ...
            $this->appendAdminLogUGD((int) $this->rjNo, $logVerb . ' Laporan Operasi — ' . ($entry['jenisTindakan'] ?: '-') . ' (' . $key . ')', 'MR');
        });
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            $pdf = Pdf::loadView('pages.components.modul-dokumen.ri.laporan-operasi-ri.cetak-laporan-operasi-ri-print', ['data' => $data])->setPaper('A4');

            $this->dispatch('toast', type: 'success', message: 'Berhasil mencetak laporan operasi.');
            return response()->streamDownload(fn() => print $pdf->output(), 'laporan-operasi-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Hapus Laporan Operasi — ' . $createdAt, 'MR');
            });
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="rm-laporan-operasi-ugd-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            wire:key="{{ $this->renderKey('modal-laporan-operasi-ugd', [$rjNo ?? 'new']) }}">
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                        wire:key="lo-ugd-display-pasien-{{ $rjNo ?? 'init' }}" />
