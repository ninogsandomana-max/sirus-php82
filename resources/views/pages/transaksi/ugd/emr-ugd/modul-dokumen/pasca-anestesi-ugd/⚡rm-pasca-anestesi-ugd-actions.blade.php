<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/pasca-anestesi-ri/rm-pasca-anestesi-ugd-actions.blade.php
...
new class extends Component {
    ...
    private string $jsonKey = 'pascaAnestesiUGD';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-pasca-anestesi-ugd']);
    ...
    public function openModal(): void
    {
        ...
        $this->dispatch('open-modal', name: "rm-pasca-anestesi-ugd-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-pasca-anestesi-ugd-{$this->rjNo}");
    }
    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $this->appendAdminLogUGD((int) $this->rjNo, 'Buka kunci Monitoring Pasca Anestesi (' . $createdAt . ') oleh ' . $pembukaKunci . ' — TTD petugas dicabut', 'MR');
            });
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'pasca-anestesi-ugd-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-5xl mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.ugd.display-pasien-ugd.display-pasien-ugd :rjNo="$rjNo"
                        wire:key="pa-ugd-display-pasien-{{ $rjNo ?? 'init' }}" />
