...
new class extends Component {
    ...
    protected function resetForm(): void
    {
        ...
        $this->sumKamarOperasi = $this->sumLainLain = $this->sumtrfRJ = $this->sumTotalRJ = 0;
    ...
    public function sumAll(): void
    {
        ...
        $this->sumKamarOperasi = (int) DB::table('rstxn_ugdoks')->where('rj_no', $rjNo)->sum('ok_price');
        $this->sumLainLain = (int) DB::table('rstxn_ugdothers')->where('rj_no', $rjNo)->sum('other_price');

        $this->sumtrfRJ = (int) DB::table('rstxn_ugdtempadmins')->where('rj_no', $rjNo)->selectRaw('nvl(sum(rj_admin + poli_price + acte_price + actp_price + actd_price + obat + lab + rad + other + rs_admin + nvl(ok,0)), 0) as total')->value('total');

        $this->sumTotalRJ = $this->sumRsAdmin + $this->sumRjAdmin + $this->sumPoliPrice + $this->sumJasaKaryawan + $this->sumJasaDokter + $this->sumJasaMedis + $this->sumObat + $this->sumLaboratorium + $this->sumRadiologi + $this->sumKamarOperasi + $this->sumLainLain + $this->sumtrfRJ;
    }
                        ...
                        <div x-data="{ tab: @entangle('activeTabAdministrasi') }"
                            ...
                            <div class="p-4 min-h-[300px]">
                                ...
                                <div x-show="tab === 'KamarOperasi'" x-cloak
                                    ...
                                    x-transition:enter-start="opacity-0 translate-y-1"
                                    x-transition:enter-end="opacity-100 translate-y-0">
                                    <livewire:pages::transaksi.ugd.administrasi-ugd.kamar-operasi-ugd :rjNo="$rjNo"
