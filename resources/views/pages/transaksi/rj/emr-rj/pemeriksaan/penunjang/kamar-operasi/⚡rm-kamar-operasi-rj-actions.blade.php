<?php
// resources/views/pages/transaksi/rj/emr-rj/pemeriksaan/penunjang/kamar-operasi/rm-kamar-operasi-rj-actions.blade.php
...
/**
 * Order Kamar Operasi dari EMR Rawat Jalan — unit mengirim pasien ke OK.
 *
 * Sejajar dengan order Laboratorium/Radiologi: dokter ruangan mengirim, petugas
 * OK yang memproses, biayanya kembali ke tagihan rawat jalan. Bedanya, OK tidak
...
new class extends Component {
    use WithRenderVersioningTrait, WithValidationToastTrait, EmrRJTrait;

    public array $renderVersions = [];
    protected array $renderAreas = ['kamar-operasi-order-modal-rj'];
    ...
    public function openModal(): void
    {
        ...
        $this->drId = (string) (DB::table('rstxn_rjhdrs')->where('rj_no', $this->rjNo)->value('dr_id') ?? '');

        $this->resetValidation();
        $this->incrementVersion('kamar-operasi-order-modal-rj');
        $this->dispatch('open-modal', name: 'kamar-operasi-order-rj');
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: 'kamar-operasi-order-rj');
        $this->resetForm();
    }
    ...
    #[On('lov.selected.order-ok-rj-tindakan')]
    ...
    public function kirimKamarOperasi(): void
    {
        ...
        for ($percobaan = 1; ; $percobaan++) {
            try {
                DB::transaction(function () use ($rjNo, $drId, $drIdOk, $diagId, $tindakan, &$okRegBaru) {
                    ...
                    $statusInduk = DB::table('rstxn_rjhdrs')->where('rj_no', $rjNo)->value('rj_status');
                    ...
                    $this->appendAdminLogRJ($rjNo, "Order Kamar Operasi No.{$okRegBaru} — operator {$drId}, anestesi {$drIdOk}" . ($diagId ? ", diagnosa pra-op {$diagId}" : '') . " — {$ringkasTindakan}", 'MR');
                });
        ...
        $this->closeModal();
        $this->dispatch('kamar-operasi-rj.updated');
...
<div>
    ...
    {{-- MODAL ORDER --}}
    <x-modal name="kamar-operasi-order-rj" size="full" height="full" focusable>
        <div class="flex flex-col h-full" wire:key="{{ $this->renderKey('kamar-operasi-order-modal-rj', [$rjNo ?: 'kosong']) }}">
                ...
                <div class="w-full space-y-5">
                    ...
                    <x-border-form title="Dokter Operasi">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <livewire:lov.dokter.lov-dokter target="order-ok-rj-operator" label="Dokter Operator"
                                    :initialDrId="$drId ?: null" wire:key="order-ok-rj-operator-{{ $rjNo }}-{{ $drId }}" />
                            ...
                            <div>
                                <livewire:lov.dokter.lov-dokter target="order-ok-rj-anestesi" label="Dokter Anestesi"
                                    :initialDrId="$drIdOk ?: null" wire:key="order-ok-rj-anestesi-{{ $rjNo }}-{{ $drIdOk }}" />
                    ...
                    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                        ...
                        <x-border-form title="Diagnosa Pra-Operasi" class="lg:col-span-1 h-full">
                            ...
                            <livewire:lov.diagnosa.lov-diagnosa target="order-ok-rj-diagnosa" label="Cari Diagnosa (opsional)"
                                :initialDiagnosaId="$diagId" wire:key="order-ok-rj-diagnosa-{{ $rjNo }}-{{ $diagId }}" />
                        </x-border-form>
                        ...
                        <livewire:lov.jasa-dokter.lov-jasa-dokter target="order-ok-rj-tindakan" label="Cari Tindakan Operasi"
                            wire:key="order-ok-rj-tindakan-{{ $rjNo }}-{{ count($selectedTindakan) }}" />
