<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/instruksi-pasca-bedah-ri/rm-instruksi-pasca-bedah-rj-actions.blade.php
...
new class extends Component {
    ...
    protected array $renderAreas = ['modal-instruksi-pasca-bedah-rj'];

    /** Key penyimpanan di datadaftarri_json */
    private string $jsonKey = 'instruksiPascaBedahRJ';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-instruksi-pasca-bedah-rj']);
    ...
    public function openModal(): void
    {
        ...
        $this->instruksiList = $this->dataDaftarPoliRJ[$this->jsonKey];
        $this->isFormLocked = $this->checkEmrRJStatus($this->rjNo) || $this->disabled;
        $this->incrementVersion('modal-instruksi-pasca-bedah-rj');

        $this->dispatch('open-modal', name: "rm-instruksi-pasca-bedah-rj-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-instruksi-pasca-bedah-rj-{$this->rjNo}");
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'instruksi-pasca-bedah-rj-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="rm-instruksi-pasca-bedah-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            wire:key="{{ $this->renderKey('modal-instruksi-pasca-bedah-rj', [$rjNo ?? 'new']) }}">
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-5xl mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.rj.display-pasien-rj.display-pasien-rj :rjNo="$rjNo"
                        wire:key="ipb-rj-display-pasien-{{ $rjNo ?? 'init' }}" />
