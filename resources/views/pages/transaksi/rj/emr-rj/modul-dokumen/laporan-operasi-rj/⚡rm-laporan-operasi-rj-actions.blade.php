<?php
// resources/views/pages/transaksi/ri/emr-ri/modul-dokumen/laporan-operasi-ri/rm-laporan-operasi-rj-actions.blade.php
// Laporan Operasi (DPJP) — PAB 7.2 & 7.4.
// Pola: multi-entri (Draft + Lanjut Isi + TTD-Kunci + Lihat read-only + tabel expandable),
// disimpan ke datadaftarri_json['laporanOperasiRJ']. Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    private string $jsonKey = 'laporanOperasiRJ';
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-laporan-operasi-rj']);
    ...
    public function openModal(): void
    {
        ...
        $this->dispatch('open-modal', name: "rm-laporan-operasi-rj-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-laporan-operasi-rj-{$this->rjNo}");
    }
    ...
    public function cetak(string $createdAt)
    {
        ...
        try {
            ...
            return response()->streamDownload(fn() => print $pdf->output(), 'laporan-operasi-rj-' . ($pasien['regNo'] ?? $this->rjNo) . '.pdf');
        } catch (\Throwable $e) {
