...
// disimpan ke datadaftarri_json (key: praAnestesiRJ). Kunci entri stabil = createdAt.
...
new class extends Component {
    ...
    public function mount(?string $rjNo = null, bool $disabled = false): void
    {
        ...
        $this->registerAreas(['modal-pra-anestesi-rj']);
    ...
    public function openModal(): void
    {
        ...
        $this->dispatch('open-modal', name: "rm-pra-anestesi-rj-{$this->rjNo}");
    }

    public function closeModal(): void
    {
        $this->dispatch('close-modal', name: "rm-pra-anestesi-rj-{$this->rjNo}");
    }
    ...
    public function bukaKunci(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $fresh = $this->findDataRJ($this->rjNo) ?: [];
                $list = is_array($fresh['praAnestesiRJ'] ?? null) ? $fresh['praAnestesiRJ'] : [];
    ...
    public function hapus(string $createdAt): void
    {
        ...
        try {
            DB::transaction(function () use ($createdAt) {
                ...
                $fresh = $this->findDataRJ($this->rjNo) ?: [];
                if (!isset($fresh['praAnestesiRJ'])) {
                    throw new \RuntimeException('Data pengkajian tidak ditemukan.');
                }

                $fresh['praAnestesiRJ'] = collect($fresh['praAnestesiRJ'])
...
<div>
    ...
    {{-- ══ MODAL FORM ══ --}}
    <x-modal name="rm-pra-anestesi-rj-{{ $rjNo ?? 'init' }}" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
            wire:key="{{ $this->renderKey('modal-pra-anestesi-rj', [$rjNo ?? 'new']) }}">
            ...
            <div class="flex-1 px-4 py-4 overflow-y-auto bg-surface-soft/70 dark:bg-gray-950/20">
                <div class="max-w-full mx-auto space-y-4">
                    ...
                    <livewire:pages::transaksi.rj.display-pasien-rj.display-pasien-rj :rjNo="$rjNo"
                        wire:key="pra-rj-display-pasien-{{ $rjNo ?? 'init' }}" />
                    ...
                    <div class="p-6 space-y-6 bg-canvas border border-hairline shadow-sm sm:p-8 rounded-2xl dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <fieldset @disabled($formReadOnly) class="space-y-6">
                            ...
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-data-dasar')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-anamnese')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-antropometri-ttv')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-jalan-nafas')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-sistem-organ')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-kesimpulan')
                            @include('pages.transaksi.rj.emr-rj.modul-dokumen.pra-anestesi-rj.rm-pra-anestesi-rj-ttd')
                                @if (!$formReadOnly)
