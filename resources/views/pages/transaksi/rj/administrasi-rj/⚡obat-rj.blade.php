...
<div class="space-y-4" wire:key="{{ $this->renderKey('modal-obat-rj', [$rjNo ?? 'new']) }}" x-data>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-5 items-start">
        ...
        <div class="lg:col-span-2 p-4 border border-hairline rounded-2xl dark:border-gray-700 bg-surface-soft dark:bg-gray-800/40" x-data
            x-on:focus-lov-obat-rj.window="$nextTick(() => {
                const fokus = () => {
                    const el = $refs.lovObatRj?.querySelector('input');
            ...
            @elseif (empty($formEntryObat['productId']))
                ...
                <div x-ref="lovObatRj"
                    x-on:keydown.enter="if (!$event.target.value?.trim()) $dispatch('administrasi-rj-goto-tab', { tab: 'Laboratorium', focus: 'focus-panel-laboratorium-rj' })">
                        ...
                        @forelse ($rjObat as $item)
                            ...
                            @if ($isEditing)
                                ...
                                <tr wire:key="obat-row-{{ $item['rjobatDtl'] }}-edit" x-data
                                    ...
                                    <td colspan="7" class="px-3 py-3">
                                        ...
                                        {{-- Kronis & Iter — hanya untuk pasien BPJS / setelah dokter menyalakan Iter.
                                             Ditaruh di baris sendiri karena kolom Qty terlalu sempit untuk menampungnya. --}}
                                        @if (
                                            ($isKronisProduct && $isBpjsPatient) ||
                                                $isIterEnabledByDokter ||
                                                ($editRow['statusKronis'] ?? 'N') === 'Y' ||
                                                ($editRow['statusIter'] ?? 'N') === 'Y')
                                            <div class="flex flex-wrap items-start gap-4 pt-3 mt-3 border-t border-hairline dark:border-gray-700">
                                                @if ($isKronisProduct && $isBpjsPatient)
                                                    <div class="flex items-center gap-2">
                                                        <x-toggle :current="$editRow['statusKronis'] ?? 'N'" trueValue="Y"
                                                            falseValue="N" wireClick="toggleStatusKronis"
                                                ...
                                                @if ($isKronisProduct && ($editRow['statusKronis'] ?? 'N') === 'Y')
                                                    <div>
                                                        <div class="text-[10px] font-semibold whitespace-nowrap {{ $editSumOk ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}"
                                                            title="{{ $editSumOk ? 'BPJS + Kronis = Qty' : 'BPJS + Kronis harus = Qty' }}">
                                                            BPJS + Kronis {{ $editSumOk ? '✓' : '✗' }}
                                                            {{ $fmtNumView($editSum) }}/{{ $fmtNumView($editQtyNum) }}
                                                        </div>
                                                        <div class="flex items-center gap-1 mt-1">
                                                            <x-text-input wire:model.live.debounce.250ms="editRow.qtyBpjs"
                                                                placeholder="BPJS" class="w-24 text-sm text-right" />
                                                            <span class="text-xs text-muted-soft">+</span>
                                                            <x-text-input
                                                                wire:model.live.debounce.250ms="editRow.qtyKronis"
                                                                placeholder="Kronis" class="w-24 text-sm text-right" />
                                                        </div>
                                                    </div>
                                                @endif
                                                @if ($isIterEnabledByDokter)
                                                    <div class="flex items-center gap-2">
                                                        <x-toggle :current="$editRow['statusIter'] ?? 'N'" trueValue="Y"
                                                            falseValue="N" wireClick="toggleStatusIter"
                                                            :disabled="$isFormLocked">
                                                            Iter
                                                        </x-toggle>
                                                    </div>
                                                ...
                                                @if (($editRow['statusIter'] ?? 'N') === 'Y')
                                                    <div>
                                                        <div
                                                            class="text-[10px] font-semibold whitespace-nowrap text-indigo-600 dark:text-indigo-400">
                                                            Jumlah Iter (kali)
                                                        </div>
                                                        <x-text-input type="number" min="0"
                                                            wire:model.live.debounce.250ms="editRow.iterQty"
                                                            class="w-24 mt-1 text-sm text-right" placeholder="0" />
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
                            ...
                            @else
                                ...
                                <tr wire:key="obat-row-{{ $item['rjobatDtl'] }}-view" x-data
                                    ...
                                    {{-- Harga --}}
                                    <td class="px-3 py-1.5 text-right text-body dark:text-gray-300 whitespace-nowrap">
